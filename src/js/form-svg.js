...
export default function draw (graphType, form, options={}) {
    ...
    static pack(data) {
        ...
        .radius(d => {
            ...
            if(typeof(d.data.label) === 'string') {
                rad = textSize(d.data.label, fontSize, font).width /2;
            ...
            .margin(d => {
                ...
                if (isContainer(d)) {
                    ...
                    if (d.data.type === 'unclear') {
                        // m.bottom = (d.data.label.split('_').length > 1) ? -6 : 0;
                        m.bottom = 0;
                    }
                }
            ...
            .nodeSize(d => {
                ...
                if (isText(d)) {
                    ...
                    case 'variable':
                        ...
                        break;
                    case 'unclear':
                        txtSz = textSize(d.data.label, fontVar.size, fontVar.family, 'normal');
                        w = txtSz.width;
                        h = txtSz.height * 0.7;
...
// chart.attr('width','100%').attr('height','100%').style('fill','rgba(255,0,0,.2)');
        ...
        const unclTxtSize = d => textSize(d.data.label, fontVar.size, fontVar.family, 'normal');
