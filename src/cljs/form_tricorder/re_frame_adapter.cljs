(ns form-tricorder.re-frame-adapter
  (:require
   ["use-sync-external-store/with-selector"
    :refer[useSyncExternalStoreWithSelector]]
...
;; Setup to integrate helix with re-frame.
...
(defn- run-reaction [^js ref]
  (let [key "__rat"
        ^js rat (aget ref key)
        on-change (fn [_]
                    ;; When the ref is updated, schedule all listeners in a
                    ;; batch
...
(defn use-reaction
  ...
  (if impl.component/*current-component*
    ...
    (let [subscribe (use-batched-subscribe reaction)
          get-snapshot (hooks/use-callback [reaction] #(run-reaction reaction))]
